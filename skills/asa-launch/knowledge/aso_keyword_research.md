...
**Как искать объём вместо этого:** App Store поиск построен на механизме **suggestions/hints** (автодополнение). Именно подсказки отражают реальный спрос в конкретном сторфронте:

```bash
# keyword_popularity.py использует hints API (suggestions autocomplete)
python3 keyword_popularity.py --seeds "lebensmittel scanner,halal check" --storefronts DE,AT,CH
```

Алгоритм:
1. Запрашивает autocomplete hints по seed-ключу → получает список реальных запросов
2. Для каждого hints-результата оценивает popularity через ASA suggestions (не searchPopularity)
3. Ключи, которые **появляются в autocomplete** = реальный спрос в данной стране
4. Ключи, которых **нет в hints** = низкий или нулевой органический объём
