...
class SearchAdsAPI:
    ...
    def get_access_token_from_client_secret(self, key_content):
        if self.access_token is None:
            ...
            # )
            if self.pem_content:
                private_key = ECC.import_key(self.pem_content)
            else:
                file_path = self.certificates_dir_path + self.pem_file_name
                private_key = ECC.import_key(open(file_path, 'rt').read())
    ...
    def create_campaign(
        self, app_id, countries, campaign_name, daily_budget, curruncy
    ):
        """
        Creates a campaign to promote an app.

        Apple removed lifetime/total budget for new campaigns (June 2026) —
        campaigns are now daily-budget-only, so no budgetAmount is sent.
        """
        data = {
            "orgId": self.org_id,
            "name": campaign_name,
