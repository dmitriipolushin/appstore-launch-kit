...
Endpoint: POST https://app-ads.apple.com/cm/api/v2/keywords/recommendation
  ?adamId=<app_id>&text=<seed>&adgroupId=<id>
...
4. Find request: /cm/api/v2/keywords/recommendation?adamId=...
...
BASE_URL  = "https://app-ads.apple.com/cm/api/v2/keywords/recommendation"
DELAY_SEC = 0.4
...
def fetch_for_seed(seed: str, storefronts: list[str]) -> list[dict]:
    ...
    body = {"storefronts": storefronts}

    resp = requests.post(BASE_URL, headers=headers, params=params, json=body, timeout=15)

    if resp.status_code == 401:
        sys.exit("ERROR: 401 — cookie expired. Refresh APPLE_SA_COOKIE from DevTools.")
    if not resp.ok:
    ...
    if data.get("status") != "success":
        print(f"  API error for '{seed}': {data}")
        return []
