...
⚠️ `searchPopularity` доступен только по своим приложениям (см. предусловие выше) и для non-US рынков остаётся вспомогательным сигналом. Ориентир для keyword field / title — присутствие в hints и ASA impressions из реальной кампании. Popularity-score использовать только как вспомогательный сигнал при US-стор.
