...
**Частые запросы — официальный Search Term Popularity (по каждой активной локали):**

```bash
python3 ~/.claude/skills/aso-monitoring/scripts/asa/platform_api.py --popularity \
  --countries US,DE --terms "кандидат 1,кандидат 2,…"
```

Ключ найден → частый запрос в этой стране (`N/5` — шкала интерфейса Apple Ads, `pop` —
1–100 по стране). «Не в топ-500» → ниже порога выборки (в DE ≈ 49), это хвост, а не ноль.
Своё приложение в нише не нужно. Раз в месяц (данные выходят 5-го) полезно сравнить топ
жанра с прошлым месяцем — видно, какие запросы вошли в голову и выпали из неё:
`--genre HEALTH_FITNESS --month 2026-07 --out prev.csv` и то же за текущий месяц.

**Пороги popularity хвоста (`getRecommendedKeywords`):**

⚠️ Доступна только по своим приложениям (см. предусловие выше); гео-точность для non-US не проверена — для DE/AT/CH это вспомогательный сигнал. Ориентир для keyword field — присутствие в hints и ASA impressions из реальной кампании.

**⚠️ Ограничения:**

| Инструмент | Что измеряет | Ограничение |
|-----------|-------------|-------------|
| Search Term Popularity (`platform_api.py`) | Популярность по стране, 1–100 | Только голова (топ-500 на жанр), 15 жанров, нет RU/BY |
| Hints | Относительную частоту в данном сторефронте | Нет абсолютного объёма |
| `getRecommendedKeywords` (`keyword_popularity.py`) | Popularity 0–100 | Только своё приложение в нише; для non-US не проверено |

Самый точный сигнал объёма — impressions в реальной кампании с правильным бидом.

**Шаг 5.3 — Отсев кандидатов**

⚠️ Не отсеивать ключ только потому, что его нет в Search Term Popularity: там одна голова. И не использовать как фильтр поле `searchPopularity` (1–5) из Impression Share отчёта — в 2026-04 оно давало одинаковые `2–3` у всех ключей DE/AT/CH/GB (причина не выяснена).

Для хвоста сигнал объёма — наличие ключа в hints/autocomplete:
...
1. **Объём есть:** ключ найден в Search Term Popularity по этой стране **или** присутствует в autocomplete нужного сторфронта (первичный фильтр объёма)
