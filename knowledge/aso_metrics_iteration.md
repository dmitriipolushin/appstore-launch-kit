...
|---|---|
| Keyword coverage | Добавлять локали (cross-localization × 10 локалей в US, по 160 chars) |
...
### ⚠️ searchPopularity в этом отчёте — не для оценки объёма
...
**Не путать с двумя другими поверхностями:**
- `getRecommendedKeywords` (GraphQL, app-ads.apple.com) — отдаёт popularity 0–100 и параметр
  `storefronts` там работает. Ограничение у него другое: данные приходят только для adamId
  **твоей** ASA-организации и только в тематике этого приложения; adamId конкурента даёт
  пустой массив с HTTP 200 (проверено 09.2026).
- Официальный Apple Search Ads API v5 — popularity не отдаёт вовсе, эндпоинтов нет.
